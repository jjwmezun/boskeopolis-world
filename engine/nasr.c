...
{
    const unsigned int pp = ( state * max_gfx_layers ) - 1;
...
)
{
    ...
    const SpriteUniforms * shader_uniforms = textures[ sprite.texture ].indexed
        ? &indexed_sprite_uniforms
        : &sprite_uniforms;
    const unsigned int shader = textures[ sprite.texture ].indexed ? indexed_sprite_shader : sprite_shader;
    SetShader( indexed_sprite_shader );
    ...
    glUniformMatrix4fv( shader_uniforms->model, 1, GL_FALSE, ( float * )( model ) );
    ...
    // Set palette ID & texture if set to indexed.
    if ( textures[ sprite.texture ].indexed )
    {
        const float palette = ( float )( sprite.useglobalpal ? global_palette : sprite.palette );
        glUniform1f( shader_uniforms->palette_id, palette );
...
)
{
    ...
    const unsigned int pp = state * max_gfx_layers + layer;
    const unsigned int p = layer_pos[ pp ];
