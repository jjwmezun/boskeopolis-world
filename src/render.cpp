...
namespace Render
{
    ...
    static int graphics_per_layer[ Unit::MAX_STATES ][ NUMBER_OF_LAYERS ];
    static int number_of_states = 0;
    static int graphics_map[ MAX_GRAPHICS ];
    static Graphic layers[ MAX_GRAPHICS ];
    static GLFWwindow * window;
    static Rect camera = { 0.0f, 0.0f, Unit::WINDOW_WIDTH_PIXELS, Unit::WINDOW_HEIGHT_PIXELS };
    ...
    static unsigned int generateShaderProgram( const char * vertex_shader, const char * fragment_shader );
    ...
    static unsigned int getTextureIDGeneric( const char * local, int indexed );
    static void setVerticesColors( const Color * top_left_color, const Color * top_right_color, const Color * bottom_left_color, const Color * bottom_right_color );
    ...
    int init()
    {
        canvas_width = Unit::WINDOW_WIDTH_PIXELS;
        canvas_height = Unit::WINDOW_HEIGHT_PIXELS;
        window = glfwCreateWindow( canvas_width * magnification, canvas_height * magnification, "Boskeopolis World", NULL, NULL );
        if ( window == NULL )
        {
            Log::sendError( "Failed to create GLFW window." );
            glfwTerminate();
            return 0;
        }
        ...
        {
            Log::sendError( "Failed to initialize GLAD." );
            return -1;
        }
        ...
        tilemap_shader = generateShaderProgram( "vertex", "tilemap" );
    ...
    void update()
    {
        ...
        for ( unsigned int i = 0; i < number_of_graphics; ++i )
        {
            ...
            unsigned int shaders[ 4 ] = { sprite_shader, rect_shader, text_shader, tilemap_shader };
            Rect c = { 0.0f, 0.0f, Unit::WINDOW_WIDTH_PIXELS, Unit::WINDOW_HEIGHT_PIXELS };
            if ( !layers[ i ].abs )
            {
                c = camera;
            }
    ...
    {
        return getTextureIDGeneric( local, 1 );
    };

    unsigned int getTextureIDNoIndex( const char * local )
    {
        return getTextureIDGeneric( local, 0 );
    };

    unsigned int getTextureIDGeneric( const char * local, int indexed )
    {
        ...
        {
            Log::sendError( "Couldn’t load texture file." );
        }
    ...
    {
        float x_adjust = 0.0f;
        const float right_edge = Unit::WINDOW_WIDTH_PIXELS * 0.75 + camera.x;
        const float left_edge = Unit::WINDOW_WIDTH_PIXELS * 0.25 + camera.x;
        const float target_right = target->right();
        ...
        {
            camera.x = max_w - Unit::WINDOW_WIDTH_PIXELS;
            camera.w = max_w;
        }

        float y_adjust = 0.0f;
        const float bottom_edge = Unit::WINDOW_HEIGHT_PIXELS * 0.75 + camera.y;
        const float top_edge = Unit::WINDOW_HEIGHT_PIXELS * 0.25 + camera.y;
        const float target_bottom = target->bottom();
        ...
        {
            camera.y = max_h - Unit::WINDOW_HEIGHT_PIXELS;
            camera.h = max_h;
        }
    };
    ...
    static void character( const Character * character, const Color * color )
    {
        ...
        bufferVertices();
        setVerticesView( character->dest_x + ( character->w / 2.0f ), character->dest_y + ( character->h / 2.0f ) );
    ...
    static unsigned int generateShaderProgram( const char * vertex_shader, const char * fragment_shader )
    {
        ...
        const unsigned int vertex_shader_id = generateShader( GL_VERTEX_SHADER, vertex_shader );
        const unsigned int fragment_shader_id = generateShader( GL_FRAGMENT_SHADER, fragment_shader );
        ...
        if( !success )
        {
            ...
            Log::sendError( "ERROR: Failed to create shader program." );
            Log::sendError( infoLog );
        }
    ...
    static unsigned int generateShader( GLenum type, const char * file )
    {
        ...
        const std::string s = IO::read( std::string( "assets/shaders/" ) + std::string( file ) + std::string( ".glsl" ) );
        const char * cs = s.c_str();
        glShaderSource( shader, 1, ( const GLchar * const * )( &cs ), NULL );
        glCompileShader( shader );
        glGetShaderiv( shader, GL_COMPILE_STATUS, &success );
        if( !success )
        {
            glGetShaderInfoLog( shader, 512, NULL, infoLog );
            Log::sendError( "ERROR::SHADER::VERTEX::COMPILATION_FAILED." );
            Log::sendError( infoLog );
        }
        return shader;
    }
    ...
    unsigned int addTilemap( const char * tileset, const Tile * tiles, int w, int h, int state_number, Layer layer )
    {
        ...
        if ( texture_data == NULL )
        {
            Log::sendError( "Couldn’t generate tilemap." );
        }
        glBindTexture(GL_TEXTURE_2D, texture_ids[ number_of_textures ] );
