...
static IntMapEntry * int_map_find_entry( const IntMap * map, int needle_number, hash_t needle_hash );
...
Value int_map_get( const IntMap * map, int key )
{
    ...
    IntMapEntry * entry = int_map_find_entry( map, key, key_hash );
    return entry->value;
};
        ...
        if ( map->count + 1 > map->capacity * 0.75 )
        {
            ...
            for ( int i = 0; i < map->capacity; ++i )
            {
                if ( map->entries[ i ].key.number != -1 )
                {
                    ...
                    new_list[ new_hash ].key.number = map->entries[ i ].key.number;
                    new_list[ new_hash ].key.hash = new_hash;
                    new_list[ new_hash ].value = map->entries[ i ].value;
                }
            }
            free( map->entries );
...
static IntMapEntry * int_map_find_entry( const IntMap * map, int needle_number, hash_t needle_hash )
{
    while ( true )
    {
        IntMapEntry * entry = &map->entries[ needle_hash ];
        if ( entry->key.number == -1 || entry->key.number == needle_number )
        {
            entry->key.hash = needle_hash;
            return entry;
        }
        needle_hash = ( needle_hash + 1 ) % map->capacity;
    }
}
